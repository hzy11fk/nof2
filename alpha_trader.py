# 文件: alpha_trader.py (完整优化版 - V-Dual-ML)
# 描述: 
# 1. (新) __init__ 现在加载两套 RF 模型：
#    - 'rule6_strategic_...pkl' (75分钟 预测) -> 用于 L3 AI (Rule 6)
#    - 'rule8_tactical_...pkl' (ATR SL/TP 预测) -> 用于 L1 Python (Rule 8)
# 2. (新) 增加了 _get_ml_prediction_rule6_strategic 和 _get_ml_prediction_rule8_tactical 两个独立的预测函数。
# 3. (新) start() 循环现在调用 _get_ml_prediction_rule6_strategic 来喂给 AI。
# 4. (新) _check_python_rule_8 现在调用 _get_ml_prediction_rule8_tactical 进行过滤。
# 5. (保留) 所有其他优化 (K线收盘价, 高频循环, AI 规则 等) 均保留。
    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                await asyncio.sleep(60)# 文件: alpha_trader.py (完整优化版 - V-Dual-ML)
# 描述: 
# 1. (新) __init__ 现在加载两套 RF 模型：
#    - 'rule6_strategic_...pkl' (75分钟 预测) -> 用于 L3 AI (Rule 6)
#    - 'rule8_tactical_...pkl' (ATR SL/TP 预测) -> 用于 L1 Python (Rule 8)
    ...
    **MANDATORY OUTPUT FORMAT:**
    ...
    1.  `"chain_of_thought"` (string): A multi-line string containing your detailed analysis in English. It MUST follow this template precisely:
        ...
        1. [SYMBOL] ([SIDE]):
           ...
           Reversal & Profit Save Check (With Memory):
           - [Check 1: Is this position *currently* significantly profitable (e.g., UPL Percent > +3.5%)?]
           - [Check 2: Has this position *previously* been significantly profitable (e.g., Peak_Profit_Achieved > 4.0%) AND has now pulled back significantly (e.g., UPL is < 60% of Peak_Profit)?]
           - [IF (Check 1 OR Check 2 is True) AND (it is showing *strong* signs of reversal, e.g., 1h RSI divergence):]
           - [Decision: MUST issue a CLOSE order to secure profits.]
